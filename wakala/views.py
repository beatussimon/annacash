...
class WakalaListView(ListView):
    ...
    def get_queryset(self):
        ...
        return Wakala.objects.filter(wakalauser__user=self.request.user).distinct()
...
class WakalaTransactionListView(ListView):
    ...
    def get_queryset(self):
        ...
        return Transaction.objects.filter(wakala_id=wakala_id).select_related(
            "financial_day", "network", "bank", "created_by"
        )
...
class WakalaTransactionCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"Transaction {txn.transaction_code} recorded successfully."
        )
...
class WakalaTransactionEditView(UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"Transaction {txn.transaction_code} updated successfully."
        )
        return redirect("dashboard:wakala", wakala_id=txn.wakala.id)
...
class WakalaDayOpenView(CreateView):
    ...
    model = FinancialDay
    fields = ["date", "opening_cash", "opening_float", "opening_balance_note"]
    ...
    def get(self, request, wakala_id, *args, **kwargs):
        ...
        if wakala.get_open_financial_day():
            messages.warning(
                request,
                f"Day for {wakala.get_open_financial_day().date} is still open.",
            )
            return redirect("dashboard:wakala", wakala_id=wakala.id)
    ...
    def form_valid(self, form):
        ...
        try:
            day = BalancingEngine.open_day(
                ...
                opening_cash=form.cleaned_data.get("opening_cash", 0),
                opening_float=form.cleaned_data.get("opening_float", 0),
                note=form.cleaned_data.get("opening_balance_note", ""),
            )

            messages.success(
                self.request, f"Financial day for {day.date} opened successfully."
            )
            return redirect("dashboard:wakala", wakala_id=wakala.id)
...
class WakalaDayCloseView(UpdateView):
    ...
    model = FinancialDay
    fields = ["closing_cash", "closing_float", "closing_balance_note"]
    ...
    def form_valid(self, form):
        ...
        try:
            day = BalancingEngine.close_day(
                ...
                closing_float=form.cleaned_data["closing_float"],
    ...
    def get_context_data(self, **kwargs):
        ...
        balance_results = self.open_day.calculate_computed_balance()
        context["computed_balance"] = balance_results["total"]
        context["computed_cash"] = balance_results["cash"]
        context["computed_float"] = balance_results["float"]
...
def transaction_detail(request, wakala_id, transaction_id):
    ...
    txn = get_object_or_404(
        Transaction.objects.select_related(
            "financial_day", "wakala", "network", "bank", "created_by"
        ),
        pk=transaction_id,
        wakala_id=wakala_id,
    )
    ...
    return render(
        request,
        "wakala/transaction_detail.html",
        {
            "transaction": txn,
            "wakala": txn.wakala,
...
def wakala_settings(request, wakala_id):
    ...
    if request.method == "POST":
        ...
        wakala.alternate_phone = request.POST.get(
            "alternate_phone", wakala.alternate_phone
        )
        ...
        wakala.requires_manager_approval = (
            request.POST.get("requires_manager_approval", "off") == "on"
        )
        wakala.save()
