...
class MchezoService:
    ...
    def create_group(admin_user, **group_data):
        ...
        # Set audit fields
        group_data['created_by'] = admin_user
        group_data['original_recorder'] = admin_user
        ...
        membership = Membership.objects.create(
            ...
            original_recorder=admin_user,
        )

        # Create admin role
        MchezoRole.objects.create(
            group=group, user=admin_user, role="admin", granted_by=admin_user
        )
    ...
    def add_member(group, user, payout_order=None, phone_number=None, created_by=None):
        ...
        if payout_order is None:
            current_max = (
                Membership.objects.filter(group=group, status="active").aggregate(
                    max_order=Max("payout_order")
                )["max_order"]
                or 0
            )
    ...
    def record_contribution(
        cycle, membership, amount, payment_method, created_by=None, **kwargs
    ):
        """
        ...
        # Automated Fines & Social Fund logic (Realistic Tanzanian Mchezo)
        is_late = contribution_week < cycle.get_current_week()
        fine_amount = cycle.group.late_fine_amount if is_late else 0
        social_fund_amount = cycle.group.social_fund_contribution
        ...
        contribution = Contribution.objects.create(
            ...
            social_fund_amount=social_fund_amount,
    ...
    def record_bulk_contribution(
        ...
        **kwargs
        ...
        for i in range(int(weeks_count)):
            ...
            # Bulk is usually advance, so not late
            contribution = Contribution.objects.create(
                cycle=cycle,
                membership=membership,
    ...
    def record_payout(
        ...
        Args:
            ...
            **kwargs: Additional details (including bid_amount)
        ...
        # Extract bidding details
        bid_amount = Decimal(kwargs.pop("bid_amount", 0))
        net_amount = amount - bid_amount
        ...
        cycle.total_payouts += (
            net_amount  # Only add the net amount to total cycle payouts
        )
        cycle.save()
    ...
    def get_cycle_progress(cycle):
        ...
        contributions_total = (
            cycle.contributions.filter(status="completed").aggregate(Sum("amount"))[
                "amount__sum"
            ]
            or 0
        )
        ...
        payouts_total = (
            cycle.payouts.filter(status="completed").aggregate(Sum("amount"))[
                "amount__sum"
            ]
            or 0
        )
        ...
        return {
            ...
            "progress_percent": (
                (payouts_made / total_members * 100) if total_members > 0 else 0
            ),
        }
    ...
    def get_defaulted_members(cycle):
        ...
        active_members = cycle.group.memberships.filter(status="active").exclude(
            id__in=cycle.contributions.filter(status="completed").values(
                "membership_id"
            )
        )
