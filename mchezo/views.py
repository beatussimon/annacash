...
class MchezoGroupCreateView(UserPassesTestMixin, CreateView):
    """Create a new mchezo group (Superadmin only)."""

    model = Group
    form_class = GroupForm
    template_name = "mchezo/group_form.html"
    ...
    def form_valid(self, form):
        try:
            group = MchezoService.create_group(
            ...
            AuditLogger.log(
                ...
                description=f"Group {group.name} created by superadmin",
    ...
    def get_context_data(self, **kwargs):
        ...
        context["page_title"] = "Create Mchezo Group"
        return context
...
class MchezoMemberListView(ListView):
    ...
    def get_queryset(self):
        ...
        return Membership.objects.filter(
            group_id=group_id, is_deleted=False, status="active"
        ).select_related("user")
...
class MchezoCycleStartView(CreateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            messages.success(
                self.request, f"Cycle {cycle.cycle_number} started for {group.name}."
            )
            return redirect("dashboard:mchezo", group_id=group.id)
...
class MchezoContributionCreateView(CreateView):
    ...
    def get(self, request, group_id, *args, **kwargs):
        ...
        if not has_mchezo_role(request.user, group, ["admin", "treasurer"]):
            messages.error(
                request, "You don't have permission to record contributions."
            )
            return redirect("dashboard:mchezo", group_id=group.id)
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            contribution_week = int(
                form.cleaned_data.get("contribution_week", cycle.get_current_week())
            )
...
class MchezoPayoutCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            messages.success(
                self.request, f"Payout of TZS {payout.amount:,.0f} recorded."
            )
            return redirect("dashboard:mchezo", group_id=group.id)
...
class MchezoPayoutUpdateView(UpdateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            messages.success(self.request, f"Payout updated successfully.")
            return redirect("dashboard:mchezo", group_id=group.id)
...
def mchezo_member_detail(request, group_id, membership_id):
    ...
    # Check permissions
    if not has_mchezo_role(
        request.user, membership.group, ["admin", "treasurer", "member"]
    ):
    ...
    contributions = Contribution.objects.filter(membership=membership).order_by(
        "-contribution_date", "-contribution_time"
    )

    payouts = Payout.objects.filter(membership=membership).order_by("-completed_date")

    return render(
        request,
        "mchezo/member_detail.html",
        {
...
def mchezo_group_settings(request, group_id):
    ...
    if request.method == "POST":
        ...
        group.contribution_amount = request.POST.get(
            "contribution_amount", group.contribution_amount
        )
        group.social_fund_contribution = request.POST.get(
            "social_fund_contribution", group.social_fund_contribution
        )
        group.late_fine_amount = request.POST.get(
            "late_fine_amount", group.late_fine_amount
        )
        group.contribution_frequency = request.POST.get(
...
class MchezoMemberEditView(UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"{membership.user.get_full_name()} updated successfully."
        )
